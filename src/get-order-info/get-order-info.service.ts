...
import { ShipmentPriorityService } from 'src/shipment_priority/shipment_priority.service';
import { Cron, CronExpression } from '@nestjs/schedule';
import { EOrderRole, EOrderStatus, EOrderUser } from 'src/order/enums/enum';
...
import { ShipRushService } from 'src/ship-rush/ship-rush.service';
...
export class GetOrderInfoService {
  ...
  async findAll(): Promise<any> {
    ...
    orderInfo.value.forEach(async (element) => {
      if (
        ...
        const shp = await this._ShipmentPriorityService.findOne(
  ...
  async createShipRushDelivery(Id: string) {
    try {
      ...
      const shp = await this._ShipmentPriorityService.findOne(order.STCODE);
